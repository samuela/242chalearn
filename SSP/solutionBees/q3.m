...
for i=1:num_seq
    ...
    allTransProbs = zeros(3,3);
    for(nt=1:numel(seq_train))
        labels = seq_train{nt}.label;
    
        for(k=1:3)
            inds = find(labels==k);
            for(id=1:numel(inds))
                indUse = inds(id);
                if(indUse == numel(labels)) continue; end;

                next = labels(indUse+1);
                allTransProbs(k,next) = allTransProbs(k,next)+1;
            end
        end
    end
    allTransProbs = bsxfun(@rdivide, allTransProbs, sum(allTransProbs,2));
    ...
    [ model_est, x0_est, P0_est, ll_iter ] = ...
      em_slds(model_init, x0_init, P0_init, { seq_test }, data_train, max_iters, conv_tol);
    ...
    [X_pf,s,W] = particle_filter_slds(num_particles, model_est, noisy_poses, x0_est, P0_est);
    ...
    for t=1:T
      for state=1:num_states
        I_state = find( s(:,t) == state );
        p_k(state,t) = sum( W(I_state,t) );
      end
      p_k(:,t) = p_k(:,t) ./ sum(p_k(:,t));
    end
